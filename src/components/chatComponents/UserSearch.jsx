					...
					<>
						...
						{selectedUsers?.map((user) => {
							return (
								<div
									key={user?._id}
									className="w-full h-16 border-slate-500 border rounded-lg flex justify-start items-center p-2 font-semibold gap-2  cursor-pointer text-black"
									onClick={() => handleCreateChat(user._id)}
								>
									<img
										className="h-12 min-w-12 rounded-full"
										src={getChatImage(authUserId) || "../../assets/group.png"}
										alt="img"
									/>
